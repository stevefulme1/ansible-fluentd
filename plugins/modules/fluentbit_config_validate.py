...
DOCUMENTATION = r"""
---
module: fluentbit_config_validate
short_description: Validate a Fluent Bit configuration file
version_added: "0.1.0"
description:
  - Runs C(fluent-bit --dry-run) against a configuration file and reports
    whether the configuration is valid.
  - This module never reports C(changed).
options:
  config_path:
    description:
      - Path to the Fluent Bit configuration file to validate.
    type: path
    required: true
  fluentbit_bin:
    description:
...
EXAMPLES = r"""
- name: Validate the fluent-bit config
...
def main():
    ...
    rc, stdout, stderr = module.run_command(
        [fluentbit_bin, "--dry-run", "-c", config_path]
    )
